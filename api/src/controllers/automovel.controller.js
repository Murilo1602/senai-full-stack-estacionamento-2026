...
const criarAutomovel = async (req, res) => {
  ...
  if (!placa || !proprietario || !tipo || !modelo || !marca) {
    return res.status(400).json({
      erro: "Campos obrigatorios: placa, proprietario, tipo, modelo, marca",
    });
  }
  ...
  try {
    ...
    return res.status(400).json({ erro: "Erro ao cadastrar veiculo", detalhe: error.message });
  }
};
...
const atualizarAutomovel = async (req, res) => {
  ...
  if (tipo) {
    ...
    if (!tiposValidos.includes(tipo)) {
      return res.status(400).json({
        erro: `Tipo invalido, use: ${tiposValidos.join(", ")}`,
      });
    }
  }
